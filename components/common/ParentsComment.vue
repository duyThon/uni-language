<template>
  <div class="carousel-parent-comment">
    <ssr-carousel :slides-per-page="this.slidesPerPage" loop show-dots>
      <slide :index="1">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Bạn Linh Hương nhà chị rất thích học ở trung tâm bên em, may quá
              con theo kịp và ham học. Bạn ấy chỉ sợ mẹ ko đủ tiền đóng cho theo
              học. không muốn nghỉ buổi nào cả chỉ khi nào quá mệt mới xin nghỉ
              hoặc gia đình có việc. Chị bảo cứ học bên này đi mẹ đóng tiền đủ
              cả năm rồi ko phải lo tiền học
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p1-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Đinh Thị Thuận</h3>
              <p>Phụ Huynh bạn Lừ Linh Hương</p>
            </div>
          </div>
        </div>
      </slide>
      <slide :index="2">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Chị đánh giá rất tốt về trung tâm. Và giới thiệu phụ huynh chị
              tham gia học tại trung tâm. Chắc chắn sẽ tiếp tục cho con tham gia
              theo học lộ trình lâu dài tại trung tâm.
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p2-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Hoài Thu</h3>
              <p>Phụ Huynh bạn Nguyễn Đức Trung</p>
            </div>
          </div>
        </div>
      </slide>
      <slide :index="3">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Trước đây, con chị rất sợ môn tiếng anh. Sau thời gian học ở Uni,
              đến nay tuy không giỏi môn này, nhưng con đã không còn sợ như
              trước. Hy vọng, sau này con sẽ ngày càng học tiếng anh tốt hơn,
              trước là để thi trên lớp, thi tốt nghiệp, sau sẽ phục vụ tốt cho
              cuộc sống của con. Thật sự cảm ơn các thầy cô đã kiên nhẫn dạy
              con, đã lắng nghe những ý kiến của phụ huynh, để phối hợp với gia
              đình giúp cho con học tốt hơn!
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p3-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Hồ Thanh Huyền</h3>
              <p>Phụ Huynh bạn Trần Minh Lâm</p>
            </div>
          </div>
        </div>
      </slide>
      <slide :index="4">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Năm thứ 4 theo học tại Trung tâm Uni language house Sơn La, tôi
              nhận thấy con tiến bộ hơn rất nhiều, được học đầy đủ 4 kĩ năng
              nghe, nói, đọc, viết theo giáo trình nước ngoài. Bên cạnh đó Trung
              tâm còn tổ chức rất nhiều chương trình ngoại khoá, hội thi để các
              con tự tin và lĩnh hội nhiều kiến thức hơn. Tôi rất tin tưởng gửi
              gắm con mình theo học tại Trung tâm.
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p4-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Cầm Thị Minh Phượng</h3>
              <p>Phụ Huynh bạn Vi Trung Dũng và Vi Trung Nghĩa</p>
            </div>
          </div>
        </div>
      </slide>
      <slide :index="5">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Sau thời gian hai bạn học ở trung tâm Uni thì chị thấy con rất hào
              hứng vui vẻ học tiếng anh con cũng được trải nghiệm nhiều điều bổ
              ích khi học tiếng anh ở đây . Với khả năng học tiếng anh của con
              được trau dồi nhiều hơn em ạ.
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p5-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Lan</h3>
              <p>Phụ Huynh bạn Tú An và Tú Uyên</p>
            </div>
          </div>
        </div>
      </slide>
      <slide :index="6">
        <div class="parents-comment-item">
          <div class="speech-bubble">
            <p>
              Chị thấy mọi cái ở trung tâm rất tuyệt, con học tốt. Con đang theo
              IELTS lên đang ấp ủ nhiều dự định lắm 😊. Mong thầy cô ở Trung tâm
              luôn sẽ luôn sát cánh bên các con, vì thầy và trò đã cùng nhau đi
              dc đoạn đường dài rồi, sắp đến lúc về đích rồi. Chúc cho thầy cô
              và các con sẽ gặt hái được nhiều thành công trên con đường phía
              trước 🎈🎊
            </p>
          </div>
          <div class="parents-comment-info">
            <div class="parents-img">
              <img src="@/assets/pics/parents/p6-circle.png" alt="" />
            </div>
            <div class="parents-info">
              <h3>Chị Thùy Dương</h3>
              <p>Phụ huynh bạn Phạm Duy Khánh</p>
            </div>
          </div>
        </div>
      </slide>
    </ssr-carousel>
  </div>
</template>

<script>
export default {
  data() {
    return {
      slidesPerPage: 2,
    }
  },

  mounted() {
    const width = window.outerWidth;
    console.log(width);
    width < 740 ? this.slidesPerPage = 1 : this.slidesPerPage = 2;
  },

  // computed: {
  //   getSlidesPerPage() {
  //   // const width = document.documentElement.clientWidth;
  //   //   // console.log(window.outerWidth);
  //   //   return width < 740 ? 1 : this.slidesPerPage;
  //   },
  // }
};
</script>

<style>
.carousel-parent-comment {
  margin: 80px;
}

.parents-comment-item {
  width: 450px;
  height: 500px;
  margin: auto;
  border-radius: 30px;
  display: flex;
  flex-direction: column;
  justify-content: space-between;
}

.speech-bubble {
  position: relative;
  background: var(--primary-color);
  border-radius: 20px;
  margin: 10px 40px;
  padding: 10px 40px;
  top: 20px;
  min-height: 240px;
  display: flex;
  align-items: center;
  color: #fff;
}

.speech-bubble:after {
  content: "";
  position: absolute;
  bottom: 0;
  left: 50%;
  width: 0;
  height: 0;
  border: 40px solid transparent;
  border-top-color: var(--primary-color);
  border-bottom: 0;
  border-left: 0;
  margin-left: -20px;
  margin-bottom: -40px;
}

.parents-img img {
  height: 180px;
}

.parents-comment-info {
  display: flex;
  align-items: center;
  justify-content: space-evenly;
  margin: 0 40px;
}

.parents-comment-info h3 {
  margin: 0;
}

.parents-comment-info p {
  margin: 10px 0;
}

/* mobile */
@media screen and (max-width: 739px) {
  .carousel-parent-comment {
    margin: 0;
  }

  .parents-comment-item {
    width: 380px;
    height: 600px
  }

  .speech-bubble {
    margin: 10px auto 40px auto;
    
    padding: 10px 20px;
    width: 80%;
    font-size: 20px;
    min-height: 300px;

  }
  .parents-comment-info {
    margin: 0;
  }
}

/* tablet */
@media screen and (min-width: 740px) and (max-width: 1023px){
  .carousel-parent-comment {
    margin: 80px 0;
  }

  .parents-comment-item {
    width: 320px;
    height: 540px;
  }

  .speech-bubble {
    margin: 10px auto;
    padding: 10px 20px;
  }

  .parents-comment-info {
    margin: 0;
  }


  .ssr-carousel-slide {
    /* margin: 0 !important; */
  }
}
</style>